...
const TBContent = styled.div<{ $inverted?: boolean }>`
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: center;
  border-radius: ${({ theme }) => theme.borderRadius};
  padding: 8px; // edit this
  width: 100%;
  max-width: 100%;
...
const TBText = styled.span`
  color: inherit;
  font-size: ${({ theme }) => theme.font.size.md};
  font-weight: ${({ theme }) => theme.font.weight.bold};
  margin: 2px 12px; // this
`;
...
const TWEET_MUTATION = gql`
  mutation TweetMutation($body: String!) {
    tweet(body: $body) {
      _id
      body
      created
...
const TweetForm: React.FC<{ afterTweetCreated: (tweet: TweetT) => void }> = ({
  afterTweetCreated,
}) => {
  const [tweetMutation] = useMutation<{ tweet: TweetT }, { body: string }>(
    TWEET_MUTATION
  );

  const [tweetBody, setTweetBody] = useState("");

  const tweet: React.FormEventHandler<HTMLFormElement> = (e) => {
    e.preventDefault();
      ...
      <ContentContainer>
        <Form onSubmit={tweet}>
          ...
          <FormContent>
            ...
            <TButton disabled={tweetBody.length === 0} type="submit">
              <TBContent>
                <TBText>Tweet</TBText>
              </TBContent>
            </TButton>
